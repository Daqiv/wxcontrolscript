...
function Moments.send(stype,content)
	...
	PageUtil.pageFind();
	--寻找朋友圈图标
	--if multiColor({{66,  218, 0xf86161},{60,  250, 0x5aed56},{78,  236, 0x3cb2ef}}) == true then
	--	touchDown(66,218);
	--	mSleep(30);
	--	touchUp(66,218);
	--	mSleep(3000);
		...
		tap(Constants.moments_friend_x, Constants.moments_friend_y);
		...
		--寻找右上角相机图标
		--if multiColor({{634,   92, 0x393a3f},{656,   82, 0xffffff},{684,  114, 0xfefefe}}) == true then
...
function Moments.dianZan()
	...
	--寻找朋友圈图标
	--x, y = findImageInRegionFuzzy("moment_icon.png", 80, 32, 206, 94, 260, 0xffffff);

	--log("Moments.dianZan" .. "|x=" .. tostring(x) .. "|y=" .. tostring(y) );
...
function Moments.pingLun()
	...
	--	log("Moments.pingLun" .. "|x=" .. tostring(x) .. "|y=" .. tostring(y) );
